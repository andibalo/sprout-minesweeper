...
export class Minesweeper {
    constructor(gridLength, mineCount) {
    ...
    calculateAdjacentMines() {
        for (let i = 0; i < this.gridLength; i++) {
            for (let j = 0; j < this.gridLength; j++) {
                ...
                if (mineCount > 0)
                    this.grid[i][j] = mineCount.toString();
            }
        }
    }
    printGrid() {
...
function askForGridLength() {
    ...
    return new Promise((resolve) => {
        const ask = () => {
            gridLengthReader.question(`Enter grid length to create n x n matrix (optional, default is ${DEFAULT_GRID_LENGTH}): `, (input) => {
                if (input.trim() === '') {
...
function askForMineCount(gridLength) {
    ...
    return new Promise((resolve) => {
        const ask = () => {
            mineCountReader.question('Enter number of mines (optional, default is grid length): ', (input) => {
                if (input.trim() === '') {
                    resolve(gridLength);
...
function main() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        gridLength = yield askForGridLength();
        mineCount = yield askForMineCount(gridLength);
